<form wire:submit.prevent="submit" class="pt-3">
    ...
    <div class="form-group {{ $errors->has('promotion.is_global_list') ? 'invalid' : '' }}">
        <input class="form-control" type="checkbox" name="is_global_list" id="is_global_list" wire:model.defer="promotion.is_global_list">
        <label class="form-label inline ml-1" for="is_global_list">{{ trans('cruds.promotion.fields.is_global_list') }}</label>
        <div class="validation-message">
            {{ $errors->first('promotion.is_global_list') }}
        </div>
        <div class="help-block">
            {{ trans('cruds.promotion.fields.is_global_list_helper') }}
        </div>
    </div>
