<form wire:submit.prevent="submit" class="pt-3">
    ...
    <div class="form-group {{ $errors->has('team_role') ? 'invalid' : '' }}">
        <label class="form-label" for="team_role">{{ trans('cruds.user.fields.team_role') }}</label>
        <x-select-list class="form-control" id="team_role" name="team_role" wire:model="team_role" :options="$this->listsForFields['team_role']" multiple />
        <div class="validation-message">
            {{ $errors->first('team_role') }}
        </div>
        <div class="help-block">
            {{ trans('cruds.user.fields.team_role_helper') }}
        </div>
    </div>
